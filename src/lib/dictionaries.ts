...
export function getDictionary(locale: Locale): Promise<LanguageCopy> {
  const dictionaries: Record<Locale, LanguageCopy> = {
    ...
    ru: {
      download: {
        apiDescription: "Получайте сборки напрямую через API v2. Полная документация доступна в",
        apiReferenceLink: "справочнике API",
        apiTitle: "Для скриптов и CI",
        bannerUnmaintained: "Больше не поддерживается · Последняя сборка {date}",
        buildCount: "{n} сборок",
        channelAll: "Все",
        ...
        copied: "Скопировано",
        copy: "Копировать",
        ctaChangelog: "Список изменений",
        ctaDownload: "Скачать JAR",
        disclaimer:
          "Отказ от ответственности: не рекомендуется для продакшн-серверов. Используйте на свой страх и риск. Команда Leaf не несёт ответственности за возможные неполадки.",
        eyebrow: "Загрузки",
        heroBuild: "Сборка #{n}",
        heroReleasedOn: "Выпущена {date}",
        intro:
          "Выберите версию Minecraft и скачайте JAR-файл сервера. Для рабочих серверов используйте стабильный канал.",
        loading: "Загрузка…",
        pagination: "Страница {page} из {total}",
        pillExperimental: "Экспериментальная",
        pillLatestExperimental: "Последняя экспериментальная",
        pillLatestStable: "Последняя стабильная",
        pillStable: "Стабильная",
        pillUnmaintained: "Не поддерживается",
        sha256Label: "sha256",
        stateApiDownBody:
          "Попробуйте позже или скачайте JAR напрямую со страницы GitHub Releases.",
        stateApiDownTitle: "Сервис загрузок временно недоступен",
        stateGithubFallback: "Перейти к GitHub Releases",
        stateNoBuildsBody: "Для этой версии Minecraft пока нет опубликованных сборок.",
        stateNoBuildsTitle: "Нет доступных сборок",
        stateNoChannelBuilds: "Нет сборок в канале «{channel}»",
        stateNoStableBody:
          "Доступны только экспериментальные сборки. Используйте с осторожностью — не рекомендуется для продакшна.",
        stateNoStableTitle: "Нет стабильных сборок",
        timelineHeading: "Недавние сборки",
        timelineSummary: "{version} · Показаны последние {count}",
        timelineViewAll: "Показать все {count} сборок для {version}",
        title: "Скачать Leaf",
        viewAllVersions: "Все версии",
      },
      home: {
        config: {
          bullets: [
            "Горячая перезагрузка — большинство параметров вступает в силу без перезапуска",
            "Тонкая настройка производительности для серверов любого типа",
            "Понятная документация с пояснением каждого параметра",
          ],
          cta: "Руководство по конфигурации",
          eyebrow: "Взгляните на конфиг",
          filename: "leaf-global.yml",
          snippet: {
            footer: "# … FIXES / NETWORK / GAMEPLAY и другие параметры не показаны",
            header: "# Конфигурация Leaf — краткий обзор",
          },
          sub: "Плоский YAML-файл, управляющий каждой деталью вашего сервера. Продуманная структура и говорящие названия параметров сохраняют конфиг чистым и понятным.",
          title: "Меняйте только то, что нужно. Остальное сделает Leaf.",
        },
        cta: {
          primary: "Скачать v{version}",
          secondary: "Быстрый старт",
          sub: "Один JAR-файл и минимум усилий. Leaf легко встраивается в вашу текущую инфраструктуру.",
          title: "Ваш сервер готов. Leaf тоже.",
        },
        faq: {
          faq: [
            {
              a: "Leaf создан на базе Paper и наследует полный Paper API. Любые плагины для Paper работают «из коробки» без лишних настроек.",
              q: "Leaf — это форк Paper?",
            },
            {
              a: "Нет. Все оптимизации сохраняют ванильную механику. Поведение игры полностью соответствует Paper. Любые исключения явно отмечены в документации.",
              q: "Заметят ли игроки изменения в поведении сервера?",
            },
            {
              a: "Да, Leaf включает все патчи Purpur, при этом часть настроек перенесена в конфигурационные файлы Gale и Leaf.",
              q: "Включает ли Leaf в себя Purpur?",
            },
          ],
          head: {
            eyebrow: "Популярные вопросы",
            sub: "Остались вопросы? Загляните в документацию~",
        ...
        features: {
          features: [
            {
              body: "Спроектирован для поддержания стабильного TPS и мгновенного отклика при высоких нагрузках.",
              title: "Создан для масштабирования",
            },
            {
              body: "Высокая производительность без компромиссов при сохранении 100% совместимости с плагинами Paper.",
              title: "Простая замена ядра",
            },
            {
              body: "Использует свежие зависимости, чтобы вы получали апстрим-фиксы и оптимизации с первого дня.",
              title: "Современный стек",
            },
          ],
          featuresHead: {
            eyebrow: "Что вы получаете",
            sub: "У Leaf множество преимуществ, но эти три — самые важные.",
            title: "Три главных достоинства. Остальное — в документации.",
          },
        },
        footer: {
          cols: [
            ...
            },
            {
              ...
              items: [
                { href: "https://discord.com/invite/gfgAwdSEuM", title: "Discord" },
                { href: "https://qm.qq.com/q/m85buUivEA", title: "Группа QQ" },
              ],
            },
            {
              h: "Ресурсы",
          ...
          copyright: "© 2026 Leaf MC. Все права защищены.",
          copyrightNote: "Контент сайта распространяется по лицензии CC BY-NC-SA 4.0.",
          tag: "Высокопроизводительное серверное ядро Minecraft с полной совместимостью с плагинами и сохранением ванильного поведения.",
        },
        hero: {
          ctaPrimary: "Начать работу",
          ctaSecondary: "Открыть на GitHub",
          meta: [
            { label: "Свежая версия", value: "{version}" },
            { label: "Активных серверов", value: "{servers}" },
            { label: "Звёзд", value: "{stars}" },
          ],
          pill: "Релиз v{version}",
          subtitle:
            "Высокопроизводительное ядро Minecraft, сохраняющее ванильные механики и полную совместимость с экосистемой Paper. Создано для серверов, которым требуется максимум отдачи.",
          titleA: "Сервер без задержек. ",
          titleB: "Только плавность",
          titleC: " и высокий TPS.",
        },
        placeholder: {
          aiLabel: "AI-заглушка",
          ariaHidden: "Заглушка раскрыта",
          ariaReveal: "Нажмите, чтобы открыть заглушку",
          badge1: "Статус: временно",
          badge2: "Автор: нейросеть",
          badge3: "Срок: пока у фронтендера нет времени",
          body: "Да, даже этот текст был написан нейросетью. Разработчик ещё не решил, что именно тут должно находиться — планы грандиозные, но дедлайн уже стучится в дверь. Чтобы не оставлять пустое место, сюда поставили меня. Как только появится вдохновение и свободная минутка, меня заменят настоящим текстом. А пока — держу оборону.",
          footer: "Остальная часть сайта: 100% ручная работа ✋",
          overlayHint: "Нажмите для просмотра →",
          overlayTitle: "Я просто заглушка",
          title: "Привет, я заглушка от ИИ",
          todo: "// TODO: дождаться вдохновения",
        },
      },
    },
    zh: {
